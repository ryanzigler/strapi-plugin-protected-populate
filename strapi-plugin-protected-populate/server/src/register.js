...
module.exports = ({ strapi }) => {
  ...
  const insertMiddlewareByPath = (routesList, path, method, data, func) => {
    ...
    while (routes.length !== 0) {
      if (
        (typeof routes[0] === 'object' && Array.isArray(routes[0])) ||
        (routes[0].routes === 'object' && Array.isArray(routes[0].routes))
      ) {
  ...
  for (const pluginName of Object.keys(strapi.plugins)) {
    insertMiddlewareByPath(
      [strapi.plugins[pluginName].routes],
      '',
      '',
      pluginName,
      savePluginRoutesWithoutPrefix
    );
  }
